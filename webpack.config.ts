...
if (!DEV_SERVER) {
  COPY_FOLDERS.unshift({ from: PROD ? 'src/index.prod.ejs' : 'src/index.dev.ejs' });
} else {
...
const commonConfig = function webpackConfig(): WebpackConfig {
  ...
  console.log('PROD='+PROD+' UNIVERSAL='+UNIVERSAL+' DEV_SERVER='+DEV_SERVER);
  config.output = {
    publicPath: PROD || UNIVERSAL ? '/topicgraph/' : '/'
  };
  ...
  config.plugins = [
    ...
    new ProvidePlugin({
      ...
      new HtmlWebpackPlugin({
        template: PROD ? 'src/index.prod.ejs' : 'src/index.dev.ejs',
        filename: 'index.html',
        favicon: 'src/assets/Favicon.png',
        inject: false,
        baseUrl: PROD ? '/topicgraph/' : '/'
      }),
    ...MY_CLIENT_PLUGINS
  ];
